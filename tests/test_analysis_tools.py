...
def test_extract_markers_by_name():
    class MockProfile(Profile):
        def __init__(self):
            self.string_table = {0: "MarkerA", 1: "MarkerB", 2: "Other"}
            self.processes = {
                1: {
                    "name": "Content",
                    "threads": [
    ...
    profile = MockProfile()
    result = extract_markers_by_name(profile, ["MarkerA", "MarkerB"])
